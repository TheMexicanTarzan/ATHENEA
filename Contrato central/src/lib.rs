...
static mut STATE:Option<Atenea> = None;
...
fn state_mut() -> &'static mut Atenea {
...
impl Atenea {
    ...
    async fn useNFT(&mut self, amount_tokens: u128){ 
...
extern "C" fn state() {
    ...
    msg::reply::<IoAtenea>(state.into(), 0)
    .expect("Failed to encode or reply with `<ContractMetadata as Metadata>::State` from `state()`");
    
}


impl From<Atenea> for IoAtenea {

    fn from(value: Atenea) -> Self {
